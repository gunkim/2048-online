...
const RoomInfo = ({ username, gameInfo, handleReady, handleStart }: RoomInfoProps) => {
  ...
  if (isHost) {
    ...
    if (isParticipant && isAllReady) {
      ...
      return (
        <Heading margin="none" textAlign="center" level={2}>모든 참가자가 준비를 완료해야 <br /> 게임을 시작할 수 있어요!</Heading>
      )
    }
  } else {
    ...
    if (isReady) {
      return (
        <>
          ...
          <Box pad="xsmall">
            <Button primary label="준비 해제" onClick={handleReady} />
          </Box>
        </>
      )
    } else {
                  ...
                  >
                    <Box>
                      <Avatar
                        ...
                        style={player.ready ? { border: "5px solid #b400ff" } : {}}
                      />
                    </Box>
                  </Box>
                </Grid>
