...
class RoomTests : StringSpec({
    "인원이 2명보다 적을 경우 예외가 발생한다" {
        assertThrows<InvalidDomainException> { Room(title = "test", gamers = listOf(), isStart = false) }
    ...
    "게임이 시작되지 않았을 경우 예외가 발생한다" {
        ...
        assertThrows<InvalidDomainException> { room.move(user1, MoveType.TOP) }
    ...
    "게임이 이미 시작된 상태에서 또 시작을 할 경우 예외가 발생한다" {
        ...
        assertThrows<InvalidDomainException> { room.start(user1) }
    ...
    "게임이 시작되지 않았는데 종료할 경우 예외가 발생한다" {
        ...
        assertThrows<InvalidDomainException> { room.stop(user1) }
                .apply { assertThat(message).isEqualTo("게임이 시작되지 않았습니다.") }
    }
    "방장이 아닌 플레이어가 시작을 할 경우 예외가 발생한다" {
        ...
        assertThrows<InvalidDomainException> { room.start(user2) }
    ...
    "방장이 아닌 플레이어가 종료를 할 경우 예외가 발생한다" {
        ...
        assertThrows<InvalidDomainException> { room.stop(user2) }
