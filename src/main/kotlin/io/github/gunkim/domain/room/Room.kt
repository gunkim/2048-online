...
data class Room(
    ...
    fun start(user: User): Room {
        ...
        if (gamers.size < 2) {
            throw IllegalStateException("게임에 참여할 수 있는 인원은 최소 2명 이상입니다.")
        }

        if (!gamers.all(Gamer::isReady)) {
            throw IllegalStateException("게임에 참여한 모든 플레이어가 준비되어야 합니다.")
        }
    ...
    fun join(user: User): Room {
        ...
        if (gamers.hasId(user.id)) {
            throw IllegalArgumentException("이미 참여한 유저입니다.")
        }
